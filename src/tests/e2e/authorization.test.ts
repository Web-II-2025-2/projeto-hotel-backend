...
describe(`CT-E2E-01: Permission Lifecycle, Admin promotes guest to Manager
  ...
  it(`Should register a new guest user, this user will attempt to create a employee and fails,
    ...
    const guestTokenRes = await request(app).post("/auth/login").send({
      email: userEmail,
      password: userPassword,
    });
    ...
    const managerLoginRes = await request(app).post("/auth/login").send({
      email: userEmail,
      password: userPassword,
    });
